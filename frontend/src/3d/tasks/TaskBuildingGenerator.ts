// Здание задачи: один генератор, декоративные пресеты по отрасли и визуальные слои по готовности.
// Размер здания не меняется — улучшается оформление (ТЗ: «не менять физический размер резко»).
// Один taskId → всегда один и тот же вариант (seed = hash(task.id)).
...
/** Высота луча-маяка по уровню: не только цвет различает уровни. */
...
export const PRESET_LABEL: Record<BuildingPreset, string> = {
  cafe: 'Кафе', shop: 'Магазин', warehouse: 'Логистический хаб', lab: 'Лаборатория', academy: 'Академия', clinic: 'Клиника', pavilion: 'Павильон',
};
...
export interface BuildingSpec {
  ...
  glow: Part[];
  props: LocalProp[];
  /** высота крыши (для маркера и маяка) */
  height: number;
  /** место флага выбранной команды (локально) */
  flagAt: [number, number];
}

const W = 9, D = 8, FZ = D / 2; // фасад на +z
...
export function generateBuilding(preset: BuildingPreset, level: LevelKey, seed: number): BuildingSpec {
  ...
  switch (preset) {
    case 'cafe': {
      ...
      gbox([0, 1.9, FZ - 0.9 + 0.02], [W - 1.6, 2.6, 0.1], pal.glass); // витрина
      box([0, 4.55, -0.3], [W + 0.3, 0.3, D - 0.3], pal.trim); // парапет
      if (L >= 1) for (let i = 0; i < 6; i++) box([-3.75 + i * 1.5, 3.55, FZ + 0.1], [1.5, 0.12, 1.7], i % 2 ? pal.trim : pal.accent, { rotX: 0.35 }); // полосатая маркиза
    ...
    case 'warehouse': {
      ...
      for (const x of [-2.2, 2.2]) {
        ...
        for (let k = 0; k < 5; k++) box([x, 0.5 + k * 0.62, FZ - 0.8], [3, 0.08, 0.06], '#8c98a6'); // рулонные ворота
      }
    ...
    case 'lab': {
      ...
      for (let i = 0; i < 7; i++) box([-3.9 + i * 1.3, 4.1, FZ - 0.85], [0.16, 5.6, 0.25], pal.wall); // вертикальные рёбра
      ...
      if (L >= 2) for (const x of [-3, -1.2]) box([x, 7.5, -1.5], [1.6, 0.1, 1.1], '#2c3e66', { rotX: -0.4 }); // солнечные панели
      break;
    }
    case 'academy': {
      height = 7.2;
  ...
  // ---- слои готовности ----
  if (L === 0) {
    // строительные леса и конусы: «задаче не хватает сведений»
  ...
  if (L >= 2) {
    box([0, 2.85, FZ + 0.25], [3.6, 0.16, 1.9], pal.accent); // козырёк над входом
    box([0, 0.2, FZ + 1.9], [3, 0.12, 2.2], '#cfd5dc');
    if (preset !== 'pavilion') box([-2.2, height + 0.5, -1.8], [1.4, 0.7, 1.4], '#b8c0ca'); // кровельное оборудование
  }
  if (L >= 3) {
    ...
    box([0, 0.16, FZ + 2.6], [1.8, 0.04, 3.4], '#b91c1c'); // ковровая дорожка
...
/** Главный павильон в центре площади: круглый стеклянный павильон с ядром готовности (узлы ТЗ 5.5 сохраняются в компоненте). */
export function generatePavilion(level: LevelKey): BuildingSpec {
  ...
  else solid.push({ kind: 'cyl', p: [0, 4.45, 0], s: [4.0, 0.12, 3.9], color: '#c9cfd6', seg: 6 }); // перекрытие без кровли
