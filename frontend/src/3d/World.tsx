...
import type { Emote, PublicTask } from '../shared/types';
...
export default function World() {
  ...
  const respondedIds = useMemo(() => new Set(me?.role === 'team' ? snap!.ownProposals.map((p) => p.taskId) : []), [snap, me?.role]);
    ...
    <div className="world-root">
      <SceneBoundary>
        ...
        >
          ...
          <Sculptures />
          <HallOfAchievements items={snap.achievements ?? []} />
          ...
          <LocalPlayer
            ...
            avatarPreset={avatarFor(me?.id ?? net?.store.me ?? 'guest')}
            teamColor={myTeam?.color ?? (me?.role === 'business' ? '#16324f' : '#9aa3b2')}
            teamName={myTeam?.name ?? null}
            displayName={me?.displayName ?? 'Гость'}
            relation="self"
            onInteract={interact}
            onEmote={emote}
      ...
      {net && (
        <WorldHUD
          ...
          onEmote={emote}
          onLogout={async () => { await api.logout(); nav('/login'); }}
        />
      )}
      ...
      {selected && <TaskDrawer task={selected} snap={snap} onClose={closeTask} />}
...
function TeamBaseLive({ team, slot, mine, net, tasks, myResponses, reduced, events }: { team: import('../shared/types').Team; slot: WorldLayout['teamSlots'][number]; mine: boolean; net: MultiplayerManager | null; tasks: PublicTask[]; myResponses: number; reduced: boolean; events: WorldEventManager }) {
...
function TeamBaseCounted({ team, slot, mine, store, tasks, myResponses, reduced, events }: { team: import('../shared/types').Team; slot: WorldLayout['teamSlots'][number]; mine: boolean; store: import('./multiplayer/PresenceStore').PresenceStore; tasks: PublicTask[]; myResponses: number; reduced: boolean; events: WorldEventManager }) {
  usePresence(store);
  let n = 0;
