// Team base right in the world: a team-colored pad, canopy, table with laptops, benches, flag,
// sign (name, confirmed points, who's online, what they're working on). Spawn point for members.
...
export const TeamBase = memo(function TeamBase({ team, slot, mine, onlineCount, workingOn, myResponses, reduced, events, celebrating, near }: Props) {
  const geo = useMemo(() => {
    ...
    // canopy: four posts and a flat team-colored roof
    ...
    <group position={[slot.x, 0, slot.z]} rotation-y={slot.rot}>
      ...
      <Html position={[0, 4.6, -0.4]} center zIndexRange={[20, 0]} style={{ pointerEvents: 'none' }}>
        <div className={`base-sign ${mine ? 'mine' : ''} ${celebrating ? 'cel' : ''}`} style={{ borderColor: team.color }}>
          <div className="base-name"><i style={{ background: team.color }}>{team.name.slice(0, 1)}</i>{team.name}{mine && <em>your team</em>}</div>
          <div className="base-meta">★ {team.confirmedPoints} confirmed · {onlineCount} online</div>
          {(mine || near) && workingOn.length > 0 && <div className="base-task">Working on: {workingOn.map((t) => `“${t.title.length > 34 ? t.title.slice(0, 32) + '…' : t.title}”`).join(', ')}</div>}
          {mine && myResponses > 0 && <div className="base-resp">Team applications: {myResponses}</div>}
        </div>
      </Html>
    </group>
  );
