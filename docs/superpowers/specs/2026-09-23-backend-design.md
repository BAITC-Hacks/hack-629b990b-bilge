...
Commands: session start/end; start task; save draft; request clarification; apply answers; confirm; publish; propose; select/reject proposal; create milestone; submit evidence; approve/return milestone. Commands return the updated screen and Russian feedback, not raw database rows. Errors carry code, message, fieldErrors and recovery hints. Mutation request IDs support safe retries for authenticated task/proposal/milestone creation. Unauthenticated team signup returns a secret join code once and must not automatically retry; a lost signup response can leave an unused demo team. This bounded demo limitation avoids caching fresh login secrets behind an unauthenticated retry key. Version checks protect edits from stale tabs.
...
UX review follow-up: version now represents editor changes only; revision tracks all task activity for public cards/events and defaults to the old version on existing rows. Proposal and milestone activity does not invalidate an editor write. Clarification sessions retain questions and answered fields across saves/restarts, plus a reviewed flag set by confirm. A fresh clarify starts an active session even on a confirmed card. Presenters supply progress, next question, saved value, input type/options and skip questions made unnecessary by explicit alternatives. Public cards expose only pending/approved counts and team status labels; evidence and feedback stay private. Submission and return invalidate public markers, and team creation invalidates the scoreboard.
