...
Лимит AI по умолчанию — 30 секунд, Git — 15 секунд. Отправка свидетельства последовательно включает Git и AI, поэтому может занять оба бюджета плюс сетевое время. Не показывайте фиктивный процент готовности. В `mode=stub` разбор возвращает пустой список, уточнение — шаблонные вопросы, проверка этапа — ручные проверки. Показывайте `warning`, сохраняйте ручной путь и кнопку явного повтора. Клиент и SDK не делают автоматических платных повторов.
