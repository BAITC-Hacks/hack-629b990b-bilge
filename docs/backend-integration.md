...
// Показать workspace.clarification.questions; mode/warning честно сообщают режим AI.
...
`invalidate` содержит **только** `{type, taskId, entityId, version, invalidate}`. `version` — версия задачи; для изменения этапа/отклика всё равно загрузите их собственную версию через HTTP. `sync.required` сообщает `{reason: 'connected', refetch: ['bootstrap','catalog','dashboard','scoreboard']}`. События не являются журналом: сообщения во время разрыва могут потеряться. Поэтому каждый `connect` и `sync.required` запускают чтение снимка, а открытый детальный экран обновляется отдельно. Для черновиков события получают только владельцы; участники этапа получают приватные обновления; гостям идут публичные инвалидации. Реальные данные всегда загружаются через авторизованный HTTP.
